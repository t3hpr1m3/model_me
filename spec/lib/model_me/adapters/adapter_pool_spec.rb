...
describe ModelMe::Adapters::AdapterPool do
  class DummyAdapter < ModelMe::Adapters::AbstractAdapter; end
  let(:spec) { ModelMe::Adapters::AdapterSpecification.new(:dev, adapter: 'dummy') }
  let(:pool) { ModelMe::Adapters::AdapterPool.new(spec) }
  before(:all) {
    ModelMe::Base.add_spec(:test, {adapter: :dummy})
    ModelMe::Base.register_adapter(:dummy, DummyAdapter)
    ModelMe::Base.set_adapter(:test)
  }

  subject { ModelMe::Adapters::AdapterPool.new(spec) }
  ...
  describe 'obtaining an adapter instance' do
    ...
    it 'should raise ConnectionTimeoutError if no connections become available' do
      ...
      lambda { subject.instance }.should raise_error(ModelMe::Adapters::ConnectionTimeoutError)
      threads.each { |t| t.join }
    end
